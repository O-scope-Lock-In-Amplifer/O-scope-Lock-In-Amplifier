...
from ps6000a.buffers import Buffer
...
from ps6000a.exceptions import PicoHandleError
...
class PS6000E(OScope):
	...
	def get_data(self) -> AcquisitionData:
		...
		while not self._ps.is_ready():
			time.sleep(0.1)
			if (time.perf_counter() - t0) > (self._setup.interval * self._setup.samples * 3 + 5.0):
				self._ps.close_unit()
